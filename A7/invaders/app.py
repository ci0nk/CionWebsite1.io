...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            self._welcometext = GLabel(text='Press SPACE to Play', \
            font_name='TimesBold.ttf',font_size=64,y=self.height//2,x=self.width//2)
    ...
    def update(self,dt):
        ...
        if self.input.is_key_pressed('') and self._state == STATE_INACTIVE:
        ...
        if self._state == STATE_CONTINUE:
            self._wave.setShip(Ship(GAME_WIDTH/2,SHIP_BOTTOM+(SHIP_HEIGHT/2),SHIP_WIDTH,SHIP_HEIGHT,'rocket.png'))
            self._text = None
            self._state = STATE_ACTIVE
    ...
    def __updating(self):
        if self._wave.getshipnone() == True and self._wave.getLives() > 0:
        ...
        elif self._wave.getshipnone() == True and self._wave.getLives() == 0:
            self._state = STATE_COMPLETE
            self._text = GLabel(text='Gamed Ended', \
            font_name='TimesBold.ttf',font_size=64,y=self.height//2,x=self.width//2)
            self.draw()

        elif self._wave.getaliennone()==True:
            self._state = STATE_WON
            self._text = GLabel(text='You have WON!!!', \
            font_name='TimesBold.ttf',font_size=64,y=self.height//2,x=self.width//2)
            self.draw()

        elif self._wave.mostbottomalien() - ALIEN_HEIGHT/2 < DEFENSE_LINE:
            self._state = STATE_COMPLETE
            self._text = GLabel(text='Gamed Ended', \
            font_name='TimesBold.ttf',font_size=64,y=self.height//2,x=self.width//2)
            self.draw()
